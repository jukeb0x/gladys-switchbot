...
module.exports = function (peripheral) {
  ...
  const constructorName = (peripheral.advertisement. manufacturerData|| '').toString('utf-8').replace('\u0000', '').toLowerCase();
console.log('switchbot module : constructorname '+constructorName);
  if ('WoHand' != constructorName) {
    return Promise.reject(macAddress + ' is not an switchbot device');
  } else {
