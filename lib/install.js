...
module.exports = function () {
  ...
  let serviceUUIDs = [shared.services.exec];
  let charUUIDs = [shared.characteristics.command];
  ...
  return switchbotScan().then((peripheralMap) => {
    return Promise.map(peripheralMap, (peripheralEntry) => {
      ...
      return switchbotConnect(peripheral).then((peripheral) => {
        return switchbotDiscoverServices(peripheral, serviceUUIDs);
      }).then((services) => {
        const characteristicsMap = new Map();
        return Promise.map(services, serviceEntry => {
          return switchbotDiscoverCharacteristics(peripheral, serviceEntry[1], charUUIDs).then((characteristics) => {
